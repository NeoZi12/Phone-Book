...
let users = [
  {
    ...
    description:
      'Haim Cohen is an Israeli chef, restaurateur, TV presenter and author of cookbooks. Culinary consultant for a Michelin star restaurant. Member of the Grill Knights.',
  },
  {
    name: 'Eyal Shani',
    ...
    description:
      "Eyal Tovia Mordechai Shani is an Israeli chef. Established the restaurants Ocean, Bruno, Zennon, The Salon, North Abraxas and SHMONE. Since 2010, participates as a judge in Keshet 12's Israeli Master Chef.",
  },
  {
    name: 'Gordon Ramsay',
    ...
    description:
      'Gordon James Ramsay is a well-known Scottish chef, one of the three British chefs whose restaurants currently hold three Michelin stars. Ramsay won a total of 16 Michelin stars for all his restaurants and currently holds 8 of them.',
  },
  {
    name: 'Assaf Granit',
    ...
    description:
      'Assaf Elkana Granit  is an Israeli chef, restaurateur, businessman and TV host, who has one Michelin star at Shavor restaurant in Paris.',
  },
];
...
function applyHoverEffect() {
  document.querySelectorAll('.contact-item').forEach(item => {
    item.addEventListener(
      'mouseover',
      () => (item.style.backgroundColor = '#AC9362'),
    );
...
function closeModel(event) {
  if (
    event.target === document.getElementById('myModel') ||
    event.target === document.getElementById('closeModelBtn')
  ) {
...
function closeInfo(event) {
  ...
  if (
    ...
    event.target === document.getElementById('closeInfoBtn')
  ) {
...
function deleteValues() {
  let del = confirm('Are you sure you want to delete all the contacts?');
  if (del) {
    users = [];
    clearContacts();
    if (users.length == 0)
...
function saveContact() {
  ...
  // Validation check
  if (!userExists(name) || name.trim() === '' || number.trim() === '') {
    if (!userExists(name)) alert('This user already exists');
    else alert('Name and Number are required.');
...
function deleteContact(index) {
  let del = confirm('Are you sure you want to delete this number?');
  if (del) {
